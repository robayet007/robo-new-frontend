...
function Checkout({ products }: { products: Product[] }) {
  ...
  useEffect(() => {
    ...
    // #region agent log
    fetch("http://127.0.0.1:7242/ingest/5953be24-521b-4618-a8c0-2a465a0f1894", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sessionId: "debug-session",
        runId: "pre-fix",
        hypothesisId: "H0",
        location: "Checkout.tsx:88",
        message: "Uddokta callback URL params",
        data: {
          invoiceId,
          status,
          allParams: Object.fromEntries(searchParams.entries()),
    ...
    if (invoiceId) {
      ...
      if (
        ...
        console.log(
          "ℹ️ URL status:",
          status,
          "- Will verify actual status via API"
        );
      ...
      if (searchParams.get("cancelled") === "true") {
        ...
        console.log(
          "   2. Webhook will handle verification automatically (in production)"
        );
  ...
  const handleUddoktaPayCallback = async (invoiceId: string) => {
    console.log(
      "🔄 handleUddoktaPayCallback called with invoiceId:",
      invoiceId
    );
    ...
    fetch("http://127.0.0.1:7242/ingest/5953be24-521b-4618-a8c0-2a465a0f1894", {
      ...
      body: JSON.stringify({
        ...
        runId: "pre-fix",
        hypothesisId: "H4",
        location: "Checkout.tsx:278",
        message: "handleUddoktaPayCallback entry (debug)",
        data: { invoiceId },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    ...
    fetch("http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        location: "Checkout.tsx:246",
        message: "handleUddoktaPayCallback entry",
        data: {
          invoiceId,
          uid,
    ...
    if (!currentUid) {
      currentUid =
        localStorage.getItem("checkout_uid") || searchParams.get("uid") || "";
      // #region agent log
      fetch(
        "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "Checkout.tsx:252",
            message: "UID restored from storage",
            data: {
              restoredUid: currentUid,
              fromLocalStorage: !!localStorage.getItem("checkout_uid"),
              fromUrl: !!searchParams.get("uid"),
            },
            timestamp: Date.now(),
            sessionId: "debug-session",
            runId: "run1",
    ...
    if (!currentUid) {
      ...
      fetch(
        "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "Checkout.tsx:260",
            message: "UID validation failed",
            data: {
              uid,
              currentUid,
              localStorageUid: localStorage.getItem("checkout_uid"),
    ...
    try {
      ...
      fetch(
        "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "Checkout.tsx:333",
            message: "Uddokta Pay verification response received",
            data: {
              verifyResponse,
              hasStatus: !!verifyResponse.status,
              hasPayment: !!verifyResponse.payment,
              paymentStatus: verifyResponse.payment?.status,
      ...
      // #region agent log
      fetch(
        "http://127.0.0.1:7242/ingest/5953be24-521b-4618-a8c0-2a465a0f1894",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            runId: "pre-fix",
            hypothesisId: "H1",
            location: "Checkout.tsx:337",
            message: "Uddokta verify response (debug)",
            data: {
              invoiceId: invoiceId.trim(),
              rootStatus: verifyResponse.status,
              paymentStatus: verifyResponse.payment?.status,
              paymentStatusField: verifyResponse.payment_status,
              keys: Object.keys(verifyResponse || {}),
            },
            timestamp: Date.now(),
          }),
        }
      ...
      const paymentStatusRaw =
        verifyResponse.payment?.status ||
        verifyResponse.status ||
        verifyResponse.payment_status ||
        (verifyResponse.payment && typeof verifyResponse.payment === "object"
          ? verifyResponse.payment.status
          : null) ||
        "UNKNOWN";
      ...
      const isResponseSuccessful =
        verifyResponse.status === true || verifyResponse.status === "true";
      ...
      console.log("📊 Response structure:", {
        ...
        paymentKeys: verifyResponse.payment
          ? Object.keys(verifyResponse.payment)
          : [],
        allKeys: Object.keys(verifyResponse),
      });
      console.log("📊 ===================================================");
        ...
        "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
        {
          ...
          body: JSON.stringify({
            ...
            data: {
              ...
              paymentObjectStatus: verifyResponse.payment?.status,
              hasPayment: !!verifyResponse.payment,
      ...
      const hasPaymentData =
        verifyResponse.payment?.amount ||
        verifyResponse.amount ||
        verifyResponse.payment?.invoice_id ||
        verifyResponse.invoice_id;
      ...
      const shouldProcess =
        (isVerified || (isPending && hasPaymentData)) &&
      ...
      fetch(
        "http://127.0.0.1:7242/ingest/5953be24-521b-4618-a8c0-2a465a0f1894",
        {
          ...
          body: JSON.stringify({
            ...
            runId: "pre-fix",
            hypothesisId: "H2",
            location: "Checkout.tsx:398",
            message: "Verification decision (debug)",
            data: {
              invoiceId: invoiceId.trim(),
              paymentStatus,
              isVerified,
      ...
      if (shouldProcess) {
        ...
        console.log(
          "✅ Payment verified by Uddokta Pay, now verifying with backend..."
        );
        console.log(
        ...
        fetch(
          "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              location: "Checkout.tsx:395",
              message: "Payment verified - sending data to backend",
              data: {
                uid,
                finalUid,
                paymentStatus,
        ...
        fetch(
          "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              location: "Checkout.tsx:415",
              message: "Backend verification response received",
              data: {
        ...
        if (response.success) {
          ...
          if (
            ...
            setPaymentResult({
              status: "success",
              message:
                "Payment already processed! Your order is being processed.",
              amount: product.price,
              remaining: balance,
              transactionId: invoiceId.trim().toUpperCase(),
              productName: product.name,
        ...
        const paymentStatus =
          typeof paymentStatusRaw === "string"
            ? (paymentStatusRaw.toUpperCase() as
                | "COMPLETED"
                | "VERIFIED"
                | "PENDING"
                | "CANCELLED"
        ...
        fetch(
          "http://127.0.0.1:7242/ingest/5953be24-521b-4618-a8c0-2a465a0f1894",
          {
            ...
            body: JSON.stringify({
              ...
              runId: "pre-fix",
              hypothesisId: "H3",
              location: "Checkout.tsx:539",
              message: "Not processing payment after verify",
              data: {
                invoiceId: invoiceId.trim(),
                paymentStatus,
                paymentStatusRaw,
        ...
        if (paymentStatus === "PENDING") {
          console.log(
            "⏳ Payment status is PENDING - payment is still processing"
          );
          console.log(
            "⚠️ NOT sending to backend - waiting for payment to be verified"
          );
          ...
          fetch(
            "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
            {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                location: "Checkout.tsx:531",
                message:
                  "Payment PENDING - NOT sending to backend, starting retries",
                data: { invoiceId, paymentStatus },
                timestamp: Date.now(),
                sessionId: "debug-session",
                runId: "run1",
                hypothesisId: "C",
          ...
          while (retryCount < maxRetries && !isRetryVerified) {
            ...
            try {
              ...
              const retryPaymentStatusRaw =
                retryResponse.payment?.status ||
                retryResponse.status ||
                "UNKNOWN";
              retryPaymentStatus =
                typeof retryPaymentStatusRaw === "string"
                  ? (retryPaymentStatusRaw.toUpperCase() as
                      | "COMPLETED"
                      | "PENDING"
                      | "CANCELLED"
                      | "UNKNOWN"
              ...
              isRetryVerified =
                retryPaymentStatus === "COMPLETED" ||
                retryPaymentStatus === "VERIFIED";
              ...
              fetch(
                "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
                {
                  ...
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    location: "Checkout.tsx:560",
                    message: "Retry verification result",
                    data: {
          ...
          if (isRetryVerified && retryResponse) {
            console.log(
              "✅ Payment verified after retries! Processing with backend..."
            );
            ...
            if (response.success) {
              ...
              fetch(
                "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
                {
                  ...
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    location: "Checkout.tsx:580",
                    message:
                      "Payment verified after retries - data sent to backend",
                    data: {
                      invoiceId,
                      retryPaymentStatus,
                      isRetryVerified,
            ...
            console.log(
              "⚠️ Payment still not verified after all retries - NOT sending to backend"
            );
            console.log("⚠️ Final status:", retryPaymentStatus);
            console.log(
              `⚠️ Tried ${retryCount} times, max retries: ${maxRetries}`
            );
            ...
            fetch(
              "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
              {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  location: "Checkout.tsx:595",
                  message: "Payment retry exhausted - NOT sending to backend",
                  data: {
                    invoiceId,
                    retryPaymentStatus,
                    isRetryVerified,
            ...
            setVerifyingPayment({
              ...
              status: "verifying",
              message:
                "Payment is still processing. Webhook will complete your order automatically.",
            });

            setTimeout(() => {
              setVerifyingPayment(null);
              alert(
                `Your payment is being processed. Transaction ID: ${invoiceId}\n\nStatus: ${retryPaymentStatus}\n\nTried ${retryCount} times. Your order will be completed automatically via webhook. Please check your order history in a few minutes.`
              );
              const params = new URLSearchParams({ productId: product.id });
              navigate(`/checkout?${params.toString()}`, { replace: true });
            }, 3000);
          ...
          fetch(
            "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
            {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                location: "Checkout.tsx:654",
                message: "Payment verification failed - NOT sending to backend",
                data: {
                  finalPaymentStatus,
                  verifyResponse,
                  hasPayment: !!verifyResponse.payment,
          ...
          setTimeout(() => {
            setVerifyingPayment(null);
            alert(
              `Payment verification failed. Status: ${finalPaymentStatus}. Please try again or contact support.`
            );
          }, 2000);
        }
      }
      ...
      setTimeout(() => {
        ...
        alert(
          "Payment verification failed: " +
            (err.message ||
              "Please try again. If the problem persists, contact support with transaction ID: " +
                invoiceId)
        );
      }, 2000);
    } finally {
  ...
  const handleUddoktaPayPayment = async () => {
    ...
    fetch("http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        location: "Checkout.tsx:578",
        message: "handleUddoktaPayPayment entry",
        data: { uid, uidLength: uid.length, uidTrimmed: uid.trim() },
        timestamp: Date.now(),
        sessionId: "debug-session",
        runId: "run1",
        hypothesisId: "B",
    ...
    fetch("http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        location: "Checkout.tsx:592",
        message: "Saving UID to localStorage before redirect",
        data: { uid: uid.trim(), saved: true },
        timestamp: Date.now(),
        sessionId: "debug-session",
        runId: "run1",
        hypothesisId: "B",
    ...
    try {
      ...
      // Clean and encode UID properly to prevent encoding issues
      const cleanUid = uid.trim().replace(/["|']/g, ""); // Remove any quotes

      // Use URLSearchParams for proper URL construction
      ...
      const redirectUrl = `${baseUrl}/checkout?${redirectParams.toString()}`;
      const cancelUrl = `${baseUrl}/checkout?${cancelParams.toString()}`;
      const webhookUrl = getBackendWebhookUrl();

      console.log("🔗 Redirect URLs:", { redirectUrl, cancelUrl });
      ...
      if (checkoutResponse.status && checkoutResponse.payment_url) {
        ...
        fetch(
          "http://127.0.0.1:7244/ingest/b45ca0c1-2c74-4e93-9f95-e1bb54c72b96",
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              location: "Checkout.tsx:620",
              message: "Redirecting to Uddokta Pay",
              data: {
                uid: uid.trim(),
                savedToLocalStorage: !!localStorage.getItem("checkout_uid"),
                redirectUrl,
              },
              timestamp: Date.now(),
              sessionId: "debug-session",
          ...
          <div className="mt-2 text-xs text-slate-600">
            ...
            {!ffNameLoading && ffName && (
              <div className="inline-flex items-center px-2.5 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-[11px] font-semibold text-emerald-700">
                ...
                <span className="mr-1 text-slate-600 font-normal">
                  Account Name:
                </span>
