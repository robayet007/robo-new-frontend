    ...
    <section id="diamonds" className="mt-4 sm:mt-5 md:mt-7 p-3 sm:p-4 md:p-6 rounded-[12px] sm:rounded-[16px] md:rounded-[18px] bg-white border border-slate-900/6 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-3">
        <div className="flex-1">
          <p className="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full bg-purple-400/14 text-purple-700 border border-purple-400/35 font-semibold text-[11px] sm:text-[12px] md:text-[13px]">Top-up list</p>
              ...
              <button
                ...
                className={`inline-flex gap-1 sm:gap-2 items-center px-2.5 sm:px-3 md:px-3.5 py-1.5 sm:py-2 md:py-2.5 rounded-lg sm:rounded-xl cursor-pointer transition-all duration-150 whitespace-nowrap text-xs sm:text-sm ${
                  selectedCategory === cat.id
                    ...
                    : 'border border-slate-900/8 bg-white text-slate-900 hover:border-purple-400/50'
                }`}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-3.5 mt-3 sm:mt-3.5">
        {filtered.map((item) => (
          <article key={item.id} className="p-3 sm:p-4 rounded-[12px] sm:rounded-[14px] border border-slate-900/6 bg-gradient-to-b from-purple-50 via-violet-50/50 to-white flex flex-col gap-2 sm:gap-2.5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
            ...
            {item.bonus ? <p className="m-0 text-xs sm:text-sm text-violet-600 font-semibold">{item.bonus}</p> : <div className="min-h-[14px] sm:min-h-[18px]" />}
